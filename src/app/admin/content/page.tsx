...
export default function ContentEditorPage() {
  ...
  // Updates a field in the currently active section
  const updateField = (field: string, value: any) => {
    if (!content) return;
    setContent({ ...content, [activeSection]: { ...content[activeSection], [field]: value } });
  };

  // Updates a nested field (e.g. hours.lunchStart)
    ...
    <div className="min-h-screen bg-[hsl(var(--background))]">
      <header className="border-b border-[hsl(var(--border))] px-8 py-4 flex items-center justify-between sticky top-0 bg-[hsl(var(--background))] z-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-sm tracking-[0.15em] uppercase text-[hsl(var(--foreground))]">Редактор контента</h1>
        </div>
            ...
            {SECTIONS.map((s) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-xs tracking-wider uppercase transition-colors duration-200 ${
                  activeSection === s.key ? "text-[hsl(var(--primary))] border-l-2 border-[hsl(var(--primary))] bg-[hsl(var(--primary))]/5" : "text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]"
                }`}
              >
...
function AboutEditor({ content, updateField, updateStat, addStat, removeStat }: { content: any; updateField: any; updateStat: any; addStat: any; removeStat: any }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <h2 className="text-lg tracking-[0.1em] uppercase text-[hsl(var(--foreground))]">О ресторане</h2>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Тексты</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Статистика</h3>
        <div className="flex items-center justify-between">
          <span />
          <button onClick={addStat} className="text-xs text-[hsl(var(--primary))] tracking-wider uppercase hover:underline">+ Добавить</button>
        </div>
        {content.stats.map((stat: any, i: number) => (
          <div key={i} className="flex items-center gap-2 p-3 border border-[hsl(var(--border))]">
            ...
            <button onClick={() => removeStat(i)} className="text-red-400 hover:text-red-300 ml-2">✕</button>
          </div>
        ))}
      </div>
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Изображения</h3>
        <FormField label="Путь к изображению" value={content.aboutImage || ""} onChange={(v: string) => updateField("aboutImage", v)} />
      </div>
    </div>
  );
}
...
function ChefEditor({ content, updateField }: { content: any; updateField: any }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <h2 className="text-lg tracking-[0.1em] uppercase text-[hsl(var(--foreground))]">Шеф-повар</h2>
      <div className="grid grid-cols-1 gap-4">
      ...
      <FormField label="Путь к изображению" value={content.image || ""} onChange={(v: string) => updateField("image", v)} />
    </div>
  );
}
...
function ContactEditor({ content, updateField, updateNested }: { content: any; updateField: any; updateNested: any }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <h2 className="text-lg tracking-[0.1em] uppercase text-[hsl(var(--foreground))]">Контакты</h2>
      <div className="space-y-4">
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Часы работы</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Бронирование</h3>
...
function SettingsEditor({ content, updateField }: { content: any; updateField: any }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <h2 className="text-lg tracking-[0.1em] uppercase text-[hsl(var(--foreground))]">Настройки сайта</h2>
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Общие</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Hero секция</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Дегустационное меню</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Винное сопровождение</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Футер</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-sm tracking-wider uppercase text-[hsl(var(--muted-foreground))]">Изображения</h3>
        <FormField label="About image" value={content.aboutImage} onChange={(v: string) => updateField("aboutImage", v)} />
        <FormField label="Chef image" value={content.chefImage} onChange={(v: string) => updateField("chefImage", v)} />
      </div>
    </div>
  );
}
...
function FormFieldSmall({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex-1 min-w-0">
      <label className="block text-[hsl(var(--muted-foreground))] text-[10px] tracking-wider uppercase mb-1">{label}</label>
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-2 py-1.5 bg-transparent border border-[hsl(var(--border))] text-[hsl(var(--foreground))] text-xs focus:outline-none focus:border-[hsl(var(--primary))] transition-colors" />
    </div>
  );
}
